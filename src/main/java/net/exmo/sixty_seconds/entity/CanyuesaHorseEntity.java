...
public class CanyuesaHorseEntity extends Horse {
    ...
    /** 基础生命上限（滴血）。装备前人留下的马铠后额外 +healthBonus（见马铠物品字段） */
    public static final double BASE_MAX_HEALTH = 20.0;
    /** 基础移动速度 */
    public static final double BASE_MOVEMENT_SPEED = 0.3;

    /** 根据是否装备前人留下的马铠刷新属性（数值取自马铠物品字段） */
    private void updateArmorAttributes() {
        float healthBonus = 0;
        float speedMul = 0F;
        if (this.getBodyArmorItem().getItem() instanceof PredecessorHorseArmorItem pred) {
            healthBonus = pred.healthBonus;
            speedMul = pred.speedMultiplier;
        }
        double maxHealth = BASE_MAX_HEALTH + healthBonus;
        double speed = BASE_MOVEMENT_SPEED * (1.0 + speedMul);
