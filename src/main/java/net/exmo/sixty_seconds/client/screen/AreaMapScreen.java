    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isInRect((int) mouseX, (int) mouseY, canvasX0, canvasY0, canvasX1 - canvasX0, canvasY1 - canvasY0)) {
            if (button == 0) {
                ...
                if (!AreaMapManager.mode3d
                        && net.exmo.sixty_seconds.client.SixtySecondsClientMapZone.isActive()
            ...
            if (button == 1 && !AreaMapManager.mode3d
                ...
                if (hasShiftDown()) {
                    // 删除半径按当前缩放折算 ~6px
                    if (net.exmo.sixty_seconds.client.SixtySecondsClientMapZone.removeMarkerNear(
                            worldX, worldZ, 6.0 / zoom * AreaMapManager.getStep())) {
                        playClick();
                    }
                } else {
                    net.exmo.sixty_seconds.client.SixtySecondsClientMapZone.addMarker(worldX, worldZ);
                    playClick();
                }
                return true;
            }
    ...
    private boolean tryCreativeTeleport(double mouseX, double mouseY) {
        ...
        if (home != null && hitsMapPoint(mouseX, mouseY, home.getX() + 0.5, home.getZ() + 0.5, ccx, ccy)) {
            net.exmo.sixty_seconds.bridge.fabric.ClientPlayNetworking.send(
                    new net.exmo.sixty_seconds.network.MapTeleportC2SPacket(
