 ...
 * 耐久与猛击（落距加伤、击退、盾牌击落）按档位区分：
 * 钢制参数与原版 Mace 一致，铁制更低、合金更高。
 */
public class SixtySecondsMaceItem extends MaceItem {
    ...
    public SixtySecondsMaceItem(Tier tier, float attackDamage, float attackSpeed, int maxDamage,
                                float smashDamageScale, float knockbackScale, float shieldChance,
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (attacker.fallDistance > 0.0F && !attacker.onGround() && !attacker.isFallFlying() && !attacker.isShiftKeyDown()) {
            ...
            target.knockback((double) (0.6F * (float) ((int) attacker.fallDistance) * f * knockbackScale),
