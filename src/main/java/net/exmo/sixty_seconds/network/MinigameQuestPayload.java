...
public class MinigameQuestPayload {
    ...
    public record OpenConfig(BlockPos pos, CompoundTag data) implements CustomPacketPayload {
        public static final Type<OpenConfig> ID = new Type<>(SixtySeconds.id("minigame_quest_open_config"));
        ...
        @Override
        public Type<OpenConfig> type() { return ID; }
    }

    /** 服务端->客户端：打开小游戏界面（冒险模式） */
    public record OpenGame(BlockPos pos, String minigameId) implements CustomPacketPayload {
        public static final Type<OpenGame> ID = new Type<>(SixtySeconds.id("minigame_quest_open_game"));
        ...
        @Override
        public Type<OpenGame> type() { return ID; }
    }

    /** 客户端->服务端：保存配置 */
    public record SaveConfig(BlockPos pos, CompoundTag data) implements CustomPacketPayload {
        public static final Type<SaveConfig> ID = new Type<>(SixtySeconds.id("minigame_quest_save_config"));
        ...
        @Override
        public Type<SaveConfig> type() { return ID; }
    }

    /** 客户端->服务端：小游戏完成通知（统一触发标识） */
    public record CompleteGame(BlockPos pos) implements CustomPacketPayload {
        public static final Type<CompleteGame> ID = new Type<>(SixtySeconds.id("minigame_quest_complete"));
        ...
        @Override
        public Type<CompleteGame> type() { return ID; }
    }
}
